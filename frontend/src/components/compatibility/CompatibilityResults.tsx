...
export function CompatibilityResults({ results }: CompatibilityResultsProps) {
  ...
  const getStatusLabel = (status: CompatibilityResult["status"]) => {
    switch (status) {
      case "compatible":
        return "Compatible";
      case "conditional":
        return "Compatible avec conditions";
      case "incompatible":
        return "Incompatible";
      default:
  ...
  const getOverallStatus = () => {
    if (results.every(r => r.status === "compatible")) {
      return {
        ...
        message: "Parfait ! Tous les composants sont compatibles entre eux.",
        color: "text-green-400"
      };
    } else if (results.some(r => r.status === "incompatible")) {
      return {
        status: "incompatible" as const,
        message: "Attention ! Certains composants ne sont pas compatibles.",
        color: "text-red-400"
      };
    } else {
      return {
        status: "conditional" as const,
        message: "Compatibilité possible avec quelques ajustements.",
  ...
  return (
    <div className="bg-cyberpunk-black/60 backdrop-blur-lg rounded-2xl border border-cyberpunk-red/30 p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-white">Résultats de compatibilité</h2>
        <div className="flex items-center space-x-2">
          <div className={`flex items-center space-x-2 px-3 py-1 rounded-full border ${getStatusColor(overallStatus.status)}`}>
            ...
            <span className="font-medium">{getStatusLabel(overallStatus.status)}</span>
          </div>
        </div>
      </div>

      {/* Overall Summary */}
      <div className={`p-4 rounded-lg border mb-6 ${getStatusColor(overallStatus.status)}`}>
        <div className="flex items-start space-x-3">
          {getStatusIcon(overallStatus.status)}
          <div>
            <h3 className="font-bold mb-2">Résumé général</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-white mb-4">Détails par composant</h3>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                {result.required_adapters && result.required_adapters.length > 0 && (
                  <div className="mb-3">
                    <h5 className="font-medium text-sm mb-2">Adaptateurs requis :</h5>
                    <div className="space-y-2">
                      {result.required_adapters.map((adapter, adapterIndex) => (
                        <div
                          key={adapterIndex}
                ...
                {result.alternative_suggestions && result.alternative_suggestions.length > 0 && (
                  <div className="mb-3">
                    <h5 className="font-medium text-sm mb-2">Alternatives suggérées :</h5>
                    <div className="space-y-2">
                      {result.alternative_suggestions.map((suggestion, suggestionIndex) => (
                        <div
                          key={suggestionIndex}
                          className="flex items-center space-x-2 p-2 bg-white/5 rounded"
                        >
                ...
                {result.processing_time_ms && (
                  <div className="text-xs opacity-60">
                    Analysé en {result.processing_time_ms}ms
                  </div>
                )}
              </div>
            </div>
